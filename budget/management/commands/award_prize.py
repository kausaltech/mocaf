...
class Command(BaseCommand):
    ...
    def award_prize(self, uuid, budget_level_identifier, api_url=None, api_token=None, dry_run=False):
        devices = [Device.objects.get(uuid=uuid)]
        prize_api = PrizeApi(api_url, api_token)
        award_prizes(budget_level_identifier, devices=devices, prize_api=prize_api, dry_run=dry_run)
