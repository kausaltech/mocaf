...
Adapted from FilterPy (https://github.com/rlabbe/filterpy)
by Jami Pekkanen 2021. MIT License.
...
class IMMEstimator(object):
    ...
    def __init__(self, filters, mu):
        ...
        # TODO: Not sure why the transition matrix is used in the
        #   first step. Seems like something fishy is going on in
        #   the mixing probability computation
        self._compute_mixing_probabilities(np.eye(self.N))
    ...
    def _compute_mixing_probabilities(self, M):
        """
        Compute the mixing probability for each filter.
        """

        self.cbar = dot(self.mu, M)
        for i in range(self.N):
            for j in range(self.N):
                self.omega[i, j] = (M[i, j]*self.mu[i]) / self.cbar[j]
